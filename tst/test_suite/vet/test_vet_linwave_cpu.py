"""
VET coupled-dynamics test (level 4): radiatively-damped acoustic wave (Davis+2012 sec 5.4).
...
the analytic damped wave to VETLinWave-davis54.dat.
...
VETLinWave-davis54.dat columns (see vet_linwave.cpp::VETLinwaveErrors):
...
def test_vet_linwave_1d():
    testutils.cleanup()  # clear any stale VETLinWave-davis54.dat
    try:
        for n in _RES:
            assert testutils.run("inputs/vet_linwave_1d.athinput",
                                 [f"mesh/nx1={n}", f"meshblock/nx1={n}"]), \
                f"run failed at N={n}"
        data = testutils.athena_read.error_dat("VETLinWave-davis54.dat")
