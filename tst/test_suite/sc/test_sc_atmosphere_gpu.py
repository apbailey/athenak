"""
SC coupling test (level 3): internally-heated atmosphere -> radiative equilibrium.
...
Three variants share the pgen:
  sc_atmosphere               single block, direct ALI loop, analytic check
  sc_atmosphere_2mb           split across 2 MeshBlocks (inter-block ghost exchange)
  sc_atmosphere_solvetransfer driven through the Driver/task list (production path)
"""
...
@pytest.mark.parametrize(
    "deck",
    [
        "sc_atmosphere.athinput",
        "sc_atmosphere_2mb.athinput",
        "sc_atmosphere_solvetransfer.athinput",
    ],
)
def test_sc_atmosphere(deck):
...
def test_sc_atmosphere_thermalization(eps):
    testutils.run(
        "inputs/sc_atmosphere_exp.athinput",
        flags=[
