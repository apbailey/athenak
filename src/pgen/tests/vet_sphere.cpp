...
//========================================================================================
//! \file vet_sphere.cpp
//! \brief Homogeneous emitting/absorbing sphere test for the multi-D VET formal solution.
//!
...
//! octant-indexed mu/wmu as VET::CalculateMoments), the angle integration cancels and this
...
//! broken (or NaN) sweep. Adapted from apb_rad for the rt-vet API (4D chi, bb source iterate,
...
//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator::VETSphere()

void ProblemGenerator::VETSphere(ParameterInput *pin, const bool restart) {
  (void)restart;
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (pmbp->pnrrad == nullptr) {
    std::cout << "### FATAL ERROR in vet_sphere: requires a <nr_radiation> block"
              << std::endl;
  ...
  par_for("vet_sph_setup", DevExeSpace(), 0,nmb1, 0,n3-1, 0,n2-1, 0,n1-1,
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    bb_a(m,0,k,j,i)  = inside ? b   : 0.0;
  });
  // vacuum: no incoming radiation (interior + ghosts start at zero)
  par_for("vet_sph_ir0", DevExeSpace(), 0,nmb1, 0,nangt1, 0,n3-1, 0,n2-1, 0,n1-1,
  ...
  auto mu_h  = pvet->pang->mu.h_view;
  auto wmu_h = pvet->pang->wmu.h_view;
  ...
  std::cout << "VET sphere: checked " << ncheck << " interior cells (r < " << r_mask
  ...
  if (!(l2_err < tol)) {                        // also fails on NaN
    std::cout << "### VET sphere test FAILED (crash-guard): L2 = " << l2_err
              << " not < " << tol << std::endl;
    std::exit(EXIT_FAILURE);
  }
  return;
