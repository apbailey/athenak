...
//========================================================================================
//! \file vet_twobeam.cpp
//! \brief Two crossing pencil beams -- mirror-symmetry test for the VET formal solution.
//!
...
//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator::VETTwoBeam()

void ProblemGenerator::VETTwoBeam(ParameterInput *pin, const bool restart) {
  (void)restart;
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (pmbp->pnrrad == nullptr) {
    std::cout << "### FATAL ERROR in vet_twobeam: requires a <nr_radiation> block"
              << std::endl;
  ...
  if (indcs.nx2 == 1) {
    std::cout << "### FATAL ERROR in vet_twobeam: needs a 2D mesh (nx2 > 1)" << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  par_for("vet_2beam_zero", DevExeSpace(), 0,nmb1, 0,n3-1, 0,n2-1, 0,n1-1,
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    chi_a(m,k,j,i)  = 0.0;
    bb_a(m,0,k,j,i) = 0.0;
  });
  par_for("vet_2beam_ir0", DevExeSpace(), 0,nmb1, 0,nangt1, 0,n3-1, 0,n2-1, 0,n1-1,
  ...
  par_for("vet_2beam_bc", DevExeSpace(), 0,nmb1, 0,n1-1,
  ...
  std::cout << "VET twobeam: max E_r = " << max_er << ", x-mirror asymmetry = "
            << rel_asym << std::endl;
  if (rel_asym > tol) {
    std::cout << "### VET twobeam test FAILED: x-mirror asymmetry " << rel_asym
              << " > tol " << tol << std::endl;
    std::exit(EXIT_FAILURE);
  }
  return;
