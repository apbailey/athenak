...
void ReportAtmosphere(Mesh *pm, nr_radiation::SC *psc, const AtmParams &ap,
  ...
  std::cout << "SC atmosphere (Eq. 30): niter=" << niter
            << " max|dS/S|=" << max_rel
            ...
            << std::endl;
  {
    ...
    std::ofstream f("sc_atm_profile.dat");
    f << "# tau  J/B  S/B  San/B   eps=" << ap.eps0 << "\n";
    for (auto &r : prof) f << r[0] << " " << r[1] << " " << r[2] << " " << r[3] << "\n";
  }

  if (!converged) {
    std::cout << "### SC atmosphere FAILED: ALI did not converge (max|dS/S|="
              << max_rel << " tol=" << psc->ali_tol << ")" << std::endl;
    std::exit(EXIT_FAILURE);
  }
  if (ap.require_analytic && (rms_J > ap.tol || npts < 4)) {
    std::cout << "### SC atmosphere FAILED: rms|J-Jan|/B = " << rms_J
              << " > tol " << ap.tol << std::endl;
    std::exit(EXIT_FAILURE);
  }
  std::cout << "SC atmosphere test PASSED" << std::endl;
}

AtmParams g_atm;
bool g_use_st = false;

void SCAtmosphereFinal(ParameterInput *pin, Mesh *pm) {
  (void)pin;
  if (!g_use_st || pm->pmb_pack->pnrrad == nullptr) return;
  nr_radiation::SC *psc = pm->pmb_pack->pnrrad;
  bool converged = psc->cnv_flag;
  ReportAtmosphere(pm, psc, g_atm, psc->last_niter, psc->last_max_rel, converged);
}

}  // namespace

//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator::SCAtmosphere()

void ProblemGenerator::SCAtmosphere(ParameterInput *pin, const bool restart) {
  (void)restart;
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (pmbp->pnrrad == nullptr) {
    std::cout << "### FATAL ERROR in sc_atmosphere: requires <nr_radiation>"
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
  nr_radiation::SC *psc = pmbp->pnrrad;
  if (!psc->use_ali) {
    std::cout << "### FATAL ERROR in sc_atmosphere: requires eps < 1 (ALI path)"
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  par_for("sc_atm_setup", DevExeSpace(), 0, nmb1, 0, (n3-1), 0, (n2-1), 0, (n1-1),
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    Real x = size.d_view(m).x1min + (i - is_l + 0.5) * size.d_view(m).dx1;
    chi_a(m,k,j,i) = ChiOfX(exp_p, chi0, Hsc, x, x1min_g);
    pl_a(m,k,j,i)  = B0;
    srad_a(m,0,k,j,i)  = B0;   // cold start S=B
    eps_a(m,k,j,i) = eps0;
  });
  par_for("sc_atm_ir0", DevExeSpace(), 0, nmb1, 0, nangt1, 0, (n3-1), 0, (n2-1), 0, (n1-1),
  ...
  if (g_use_st) {
    ...
    pgen_final_func = SCAtmosphereFinal;
    return;
  }
  ...
  Kokkos::fence();
  bool converged = (niter + 1 >= psc->itermin && max_rel <= psc->ali_tol);
  ReportAtmosphere(pmy_mesh_, psc, g_atm, niter + 1, max_rel, converged);
}
