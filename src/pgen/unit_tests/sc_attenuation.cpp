...
//========================================================================================
//! \file sc_attenuation.cpp
//! \brief Unit test for the nr_radiation SC 1D short-characteristics sweep.
//!
...
//! apb_rad for the rt-vet API (4D chi/srad, octant-indexed mu, FormalSolution).
...
//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator::SCAttenuation()

void ProblemGenerator::SCAttenuation(ParameterInput *pin, const bool restart) {
  (void)restart;
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (pmbp->pnrrad == nullptr) {
    std::cout << "### FATAL ERROR in sc_attenuation: requires a <nr_radiation> block"
              << std::endl;
  ...
  par_for("sc_att_setup", DevExeSpace(), 0, nmb1, 0, (n3-1), 0, (n2-1), 0, (n1-1),
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    chi_a(m,k,j,i) = chi;
    srad_a(m,0,k,j,i)  = 0.0;
  });
  // zero the intensity field
  par_for("sc_att_ir0", DevExeSpace(), 0, nmb1, 0, nangt1, 0, (n3-1), 0, (n2-1), 0, (n1-1),
  ...
  par_for("sc_att_bc", DevExeSpace(), 0, nmb1, 0, nangt1,
  ...
  if (max_err > tol) {
    std::cout << "### SC attenuation test FAILED: max rel err = " << max_err
              << " > tol " << tol << std::endl;
    std::exit(EXIT_FAILURE);
  }
  std::cout << "SC attenuation test PASSED: max rel err = " << max_err << std::endl;
}
