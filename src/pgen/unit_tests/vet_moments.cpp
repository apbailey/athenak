...
//! \brief Unit test for VET::CalculateMoments (angular moments of the radiation field).
//!
...
//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator::VETMoments()

void ProblemGenerator::VETMoments(ParameterInput *pin, const bool restart) {
  (void)restart;
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (pmbp->pnrrad == nullptr) {
    std::cout << "### FATAL ERROR in vet_moments: requires a <nr_radiation> block"
              << std::endl;
  ...
  par_for("vet_mom_init", DevExeSpace(), 0, nmb1, 0, nangt1, ks, ke, js, je, is, ie,
  ...
  if (max_err > tol) {
    std::cout << "### VET moments test FAILED: " << names[bad] << " off by "
              << max_err << " > tol " << tol << std::endl;
    std::exit(EXIT_FAILURE);
  }
  std::cout << "VET moments test PASSED: max abs err = " << max_err << std::endl;
}
