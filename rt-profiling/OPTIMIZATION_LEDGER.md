...
| I2 | **Coalesce the footpoint gather via angle-innermost `ir`** — angle the contiguous dim, warp varies over angle | The wavefront gather is memory-latency-bound; angle-innermost → coalesced `ir` + `chi`/`srad` broadcast. **WAVEFRONT-ONLY:** the diagonal (warp=cells, angle=team; occupancy wall) and `get_moments` (warp=`i`) are *pessimized* by it. | **MEASURED (prototype, `wavefront_coalesced`): sweep kernel 1.5×–1.9× faster** (16³→128³, nmu=6; nmu=3 1.54×) — prize confirmed ≫25%. Bit-identical (hash == wavefront). | **Very invasive** *native* change: `ir` rides generic CC/AMR machinery (angle=`extent_int(1)` in 5 shared subsystems) + must reorder `get_moments`; wavefront-only. Prototype's transpose tax makes its *whole-run* a net slowdown (not for production). | High | 📊 (prize confirmed; native path justified but a large wavefront-specialized project — `a100/wavefront_coalesced/SUMMARY.md`, HEAD `2889f282`. `UpdateCellSC<ANG_INNER>` seam in place.) |
