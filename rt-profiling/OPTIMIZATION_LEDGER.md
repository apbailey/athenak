...
|--:|------|--------------------|-----------------|--------------------|:------:|:------:|
| I1 | **De-starve the diagonal on big blocks** — add a per-plane spatial **tiling factor** to the league so `league = nmb·nang_tot·ntile` (each team owns a plane *tile*, not the whole plane) | Diagonal already wins on small blocks; its only failure is team starvation at large `B` (T2). Fixing it could give **one sweep that wins across the whole block ladder** and removes the crossover. | Med–High on big blocks (close the diagonal's 88³/176³ crater, ~2× there; could beat wavefront everywhere) | **Portable** (stays in Kokkos `TeamPolicy`). Risk: intra-plane cells sharing a footpoint across tiles need the *previous* plane done — safe because tiles of the **same** plane are independent; only the `h`-barrier must stay team-global, so tiling must not split a plane's dependency across teams (design carefully). | Med | 💡 (recommended next) |
| I2 | **Coalesce the footpoint gather via `ir` layout** — transpose `ir` to make **angle the innermost/contiguous** dim (`(m,k,j,i,angg)`), angle as the fastest loop index | The wavefront walks anti-diagonal cells, so consecutive threads' `ir` reads are ~`nx1` apart → uncoalesced; angle is fully independent with 8–168 values. Angle-innermost → coalesced `ir`, `chi`/`srad` become broadcasts. Directly attacks the ~16–20 % latency-bound cost. | **High** if memory-latency-bound (the data says it is) | Portable, **but a global layout change** touching bvals/moments/outputs; re-introduces the `lmin` axis branch as warp divergence — needs A/B. | High | 💡 |
